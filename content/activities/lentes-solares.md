---
title: "Lentes Solares"
duration: "30 - 45 min"
layout: "layouts/activity.njk"
permalink: "/activities/lentes-solares/"
description: "Crea tus propios lentes de Sol para proteger tus ojos de los rayos UV. Esta actividad enseña la importancia de proteger los ojos del Sol, especialmente al usar cocinas solares o durante actividades al aire libre."
summary: "Crea tus propios lentes de Sol para proteger tus ojos de los rayos UV. Esta actividad enseña la importancia de proteger los ojos del Sol, especialmente al usar cocinas solares o durante actividades al aire libre."
version: "1.0"
Origin: "SOLARAFRICA.CH"
# original: "False" # 
author:
  - scoutsgosolar@solafrica.ch
  - gosolar@scouts.org.ar
  - tributierra@scouts.org.ar
  - Mariano S. Cosentino
source:
  - SOLARAFRICA.CH
  - Tribu Tierra - Scouts de Argentina
image: "/imagenes/actividades/lentes-solares.png"
branch:
  - Naturaleza
age: "5-22"
age_group:
  - castores
  - manada
  - scouts
  - caminantes
  - rovers
materials:
  - Impresión o dibujo de lentes solares
  - lámina de plástico oscura con protección UV
  - pegamento o cinta
tools:
  - Tijeras
competencies:
  - Sostenibilidad y Medio Ambiente
  - Creatividad y Expresión
  - Salud, Bienestar y Seguridad
responsible:
  - Sinodal o Dirigente a cargo
sdgs:
  - 3
  - 13
activity_badges:
  - gosolar
activity_code: "gosolar-X06"
rama_lobatos:
  - 9
  - 47
rama_scouts:
  - 3
  - 13
rama_caminantes:
  - 3
  - 49
rama_rovers:
  - 3
  - 30
image_prompt: "An Argentine scout room, with a table in the center-left covered with printed sunglasses templates, sheets of UV-protective dark plastic, scissors, glue, and markers. A finished pair of decorated sunglasses. The left side has space for text. On the far right side, three tiny young scouts (5-10) sit at the table, one cutting, another gluing, a third decorating, seen from a distance. The UV protection craft environment dominates."
tags:
  - gosolar
  - lentes de Sol
  - UV
  - protección ocular
  - manualidad
  - seguridad solar
  - Go Solar
location: "Interior"
participants: "4-15"
complexity_execution: 1
complexity_organizer: 2
accessibility:
  - -Visual
activity_type:
  - Arte
  - Manualidad
  - Seguridad
category:
  - Salud
  - Arte
  - Seguridad
skills:
  - Conciencia ambiental
  - Creatividad
  - Seguridad
main_type: "Aprender haciendo"
space_required: "Salón amplio"
special_materials:
  - cartulina
  - pegamento
prep_time: "15 min"
scout_values:
  - Responsabilidad
  - Autocuidado integral
  - Superación personal
scout_method:
  - Aprender haciendo
ludic_component: 4
reflective_component: 2
active_component: 1
# ===== INFORMACIÓN DE FOOTER =====
# ===== INFORMACIÓN DE FOOTER =====
references:
  - title: "Manual de Energia Solar - Desafio Scouts Go-Solar"
    author: "Tribu Tierra"
    year: 2021
    series: "Mundo Mejor"
    issue: 1
    location: "Buenos Aires, Argentina"
    url: "https://scouts.org.ar/biblioteca/1393/politicas-de-educacion-social/29163/politica-de-adultos-en-el-movimiento-scout-version-2021.pdf"
  - title: "Mundo Mejor"
    author: "Tribu Tierra"
    location: "Buenos Aires, Argentina"
    url: "https://scouts.org.ar/soy-joven"

---
# Lentes Solares

## Resumen
Crea tus propios lentes de Sol para proteger tus ojos de los rayos UV. Esta actividad enseña la importancia de proteger los ojos del Sol, especialmente al usar cocinas solares o durante actividades al aire libre.

## Preparacion
- Imprime o dibuja los lentes en un papel resistente.
- Obtener lámina oscura con protección UV (los locales de polarizaods de autos generalmente tiene retazaos de film que puedes pedir)

## Desarrollo
- Recorta las dos caras por los bordes de los lentes y los dos orificios centrales.
- Pega una lámina oscura con protección UV en los orificios y luego une las dos caras.
- Deja secar unos minutos y decora los lentes como desees.
- Usa los lentes durante actividades al aire libre o al trabajar con luz solar directa.

## Cierre
- Reflexiona sobre la importancia de proteger los ojos del Sol.
- Pregunta: ¿Qué otras formas de protección solar conocen?


Precauciones:
- Asegúrate de que la lámina oscura tenga protección UV.
