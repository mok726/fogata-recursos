---
title: "Hogar dulce hogar"
layout: "layouts/activity.njk"
permalink: "/activities/hogar-dulce-hogar/"
description: "Investiga las adaptaciones de las especies acuáticas."
summary: "Investiga las adaptaciones de las especies acuáticas."
version: "DRAFT"
Origin: "YUNGA"
# original: "False" # 
author:
  - Mariano Cosentino
source:
  - YUNGA
image: "/imagenes/actividades/hogar-dulce-hogar.png"
branch:
  - Naturaleza
age: "14-17 años"
age_group:
  - caminantes
materials:
  - Acceso a internet (opcional)
  - Materiales para el póster (papel
  - lápices
  - etc.)
competencies:
  - Investigación científica
  - Conciencia ambiental
  - Creatividad
responsible:
sdgs:
  - 6
  - 14
activity_badges:
  - agua
activity_code: "agua-X22"
rama_lobatos:
  - 9
rama_scouts:
  - 6
  - 14
rama_caminantes:
  - 3
  - 6
  - 14
rama_rovers:
  - 6
  - 14
image_prompt: "An Argentine scout room, with a large poster in the center-left comparing freshwater and saltwater adaptations. Left side: freshwater fish (osmoregulation diagram), right side: salmon (anadromous), eel (catadromous), marine fish (salt excretion). Drawings of deep sea creatures (bioluminescence). The left side has space for text. On the far right side, two tiny older scouts (14-17) stand, one pointing at the salmon, the other at the bioluminescent fish, seen from a distance. The aquatic adaptations poster environment dominates."
tags:
  - agua
  - adaptaciones
  - agua dulce
  - agua salada
  - especies
  - póster
  - Insignia del Agua
location: "Interior"
participants: "4-15"
complexity_execution: 2
complexity_organizer: 2
accessibility:
  - -Visual
activity_type:
  - Investigación
  - Arte
category:
  - Biología
  - Ecología
  - Agua
skills:
  - Investigación científica
  - Conciencia ambiental
  - Creatividad
main_type: "Investigación"
space_required: "Salón amplio"
special_materials:
  - acceso a internet
  - papel grande
  - lápices
  - colores
prep_time: "25 min"
scout_values:
  - Superación personal
scout_method:
  - Educación por la acción
ludic_component: 2
reflective_component: 4
active_component: 1
# ===== INFORMACIÓN DE FOOTER =====
# references:
#   - title: "Pollinators Challenge Badge"
#     author: "FAO"
#     year: 2021
#     series: "YUNGA Learning and Action Series – Challenge badges"
#     issue: 12
#     location: "Rome"
#     url: "https://doi.org/10.4060/cb4803en"
#
# translation_notice:
#   es: "Esta traducción no fue creada por la FAO. La FAO no es responsable del contenido ni de la precisión de esta traducción. La edición original en inglés será la edición autorizada."
#   en: "This translation was not created by FAO. FAO is not responsible for the content or accuracy of this translation. The original ENGLISH edition shall be the authoritative edition."
#
# license:
#   name: "Creative Commons BY-NC-SA 3.0 IGO"
#   url: "https://creativecommons.org/licenses/by-nc-sa/3.0/igo/"
#   image: "https://licensebuttons.net/l/by-nc-sa/3.0/88x31.png"
#
# attributions:
#   - entity: "FAO"
#     role: "Concepto original"
#     year: 2021
#   - entity: "Mariano"
#     role: "Adaptación y compilación"
#     year: 2025

---

# Hogar dulce hogar

## Resumen
Investiga las adaptaciones de las especies acuáticas.

## Desarrollo
Haz una investigación para aprender sobre las adaptaciones especiales de las especies que viven en el agua dulce. Compara estas adaptaciones con aquellas de las especies que viven en el agua salada. Crea un póster con etiquetas para comparar las especies acuáticas de agua dulce y salada y sus adaptaciones típicas. También puedes estudiar otros organismos que habitan en el agua y sus adaptaciones, por ejemplo, criaturas del mar profundo o microbios (insectos) que viven en las fumarolas de vapor o en el hielo.

## Cierre
Reflexiona sobre la diversidad de vida acuática. Preguntas para discutir: ¿Qué adaptaciones te parecieron más interesantes? ¿Cómo sobreviven estas especies en su entorno?